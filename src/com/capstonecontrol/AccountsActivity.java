		...
		if (accounts.size() == 0) {
			...
			connectButton.setOnClickListener(new OnClickListener() {
				public void onClick(View v) {
					...
					prefs.edit()
							.putString(Util.CONNECTION_STATUS, Util.CONNECTING)
							.commit();
					// Get account name
			...
			if (account.name.equals(accountName)) {
				if (Util.isDebug(mContext)) {
					...
					String authCookie = "dev_appserver_login=" + accountName
							+ ":false:" + accountName;
					prefs.edit().putString(Util.AUTH_COOKIE, authCookie)
							.commit();
					C2DMessaging.register(mContext, Setup.SENDER_ID);
				} else {
					...
					mgr.getAuthToken(account, "ah", null, activity,
							new AccountManagerCallback<Bundle>() {
								public void run(
									...
									// invalidating it and
									// obtaining a new one
									mgr.invalidateAuthToken(account.type,
											authToken);
									mgr.getAuthToken(
											account,
											"ah",
